/*******************************************************************************
 *   multi_producer_consumer.c - exercise 5                                    *
 *   Author: Ariel                                                             *
 *   Reviewer: Eli                                                             *
 * ****************************************************************************/
...
int main(void)
{
    ...
    /* Initializing mutex */
    if(SUCCESS != pthread_mutex_init(&cons, NULL))
    {
    ...
    if(SUCCESS != pthread_mutex_init(&prod, NULL))
    {
        perror("pthread_mutex_init");
        FSQDestroy(fsq);
        return SYS_FAIL;
    ...
    if(SUCCESS != pthread_mutex_destroy(&cons))
    {
    ...
    if(SUCCESS != pthread_mutex_destroy(&prod))
    {
        perror("pthread_mutex_destroy");
        FSQDestroy(fsq);
        return SYS_FAIL;
...
{    
    ...
    while(rounds)
    {
        ...
        if(SUCCESS != pthread_mutex_lock(&cons))
        {
        ...
        if(SUCCESS != pthread_mutex_unlock(&cons))
        {
...
{    
    while(write_ind < ARR_SIZE)
    {
        ...
        if(SUCCESS != pthread_mutex_lock(&prod))
        {
            perror("pthread_mutex_lock");
            return NULL;
        }
        ...
        if(SUCCESS != pthread_mutex_unlock(&prod))
        {
            perror("pthread_mutex_unlock");
            return NULL;
        }
