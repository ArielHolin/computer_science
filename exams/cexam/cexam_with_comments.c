...
/*ll: maybe give meaningful names like: BASE10...*/
#define TEN   10
#define EIGHT 8

/*ll: maybe for negative the flag should be -1*/
enum sgn_values
{
	POSITIVE = 0,
	NEGATIVE
...
/*ll: you can use a single mask of 0x33 instead of two masks of 0xo3 and 0x30*/
	byte = (byte & 0x30) << 2 | (byte & 0xc0) >> 2 |
	       (byte & 0x03) << 2 | (byte & 0x0c) >> 2;
/*ll: same as above - single mask that has 1's in every second bit*/
	byte = (byte & 0x40) << 1 | (byte & 0x80) >> 1 |
	       (byte & 0x10) << 1 | (byte & 0x20) >> 1 |
	       (byte & 0x04) << 1 | (byte & 0x08) >> 1 |
	       (byte & 0x01) << 1 | (byte & 0x02) >> 1;
...
/******************* Question 9 ***********************************************/
/*ll: I think it has to do with the machines architacture. 
one machin will "read" the info from msb to lsb aka little endian,
and the other in big endian.*/


/* When creating a binary file, one stores the data inside the hardisk. 
   Storing data inside the hardisk follows is being applied in a special manner
   that is hardware specific.
   Thus when trying to read the data from a second computer with different 
   hardware, the data might follow different patterns which are specific
   to the other hardware, thus the data itself might look different
